// name: chuck midiExample.ck
// desc: example to render a MIDI file using ChucK
// see https://github.com/ccrma/chuck/blob/main/src/core/midiio_rtmidi.cpp
...
if( me.args() == 0 ) me.sourceDir() + "threenotes.mid" => filename;
...
fun void doTrack( int track, float speed )
{
    ...
    while( min.read( msg, track ) )
    {
        ...
        0=> found;
        // catch NOTEON channel 0 messages (lower nibble == 0x90)
        
        if( !found && (msg.data1 & 0xF0) == 0x90 && msg.data2 > 0 && msg.data3 > 0 )
        {
        ...
        // other messages
        // catch NOTEOFF channel 0 messages (lower nibble == 0x80)
        if(!found && ((msg.data1 & 0xF0) == 0x90) && msg.data2 > 0  )
        {
            // get the pitch and convert to frequencey; set
           // msg.data2 => Std.mtof => s[v].freq;
            // velocity data; note on
           ...
           //(v+1)%s.size() => v;
            ...
            // log
            cherr <="NOTE OFF track:" <= track <= " pitch:" <= msg.data2 <=" velocity:" <= msg.data3 <= IO.newline(); 
            1 => found;
            <<< msg.when >>>;
        }
        if (!found)
